...
def copy_wheels_to_root(notebook_dir: pathlib.Path, site: pathlib.Path) -> None:
    ...
    marimo bundles a notebook-local import into a wheel and references it
    with a relative URL that gets resolved in the browser, not baked in
    at build time. In a nested multi-notebook layout that relative
    reference has been observed to resolve one directory level higher
    than intended -- landing on the site root instead of this notebook's
    own subdirectory. Having the wheel exist at both places means that
    wrong resolution still finds the real file instead of 404ing -- see
    nginx.conf's /public/ location for the other half of this
    belt-and-suspenders fix. This template has no local-module notebook
    today, but a dropped-in one might, so this stays defensive.
    """
